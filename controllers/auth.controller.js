		...
		.then((savedUser) => {
			...
			if (savedUser) {
				if (savedUser.Email) {
					...
					return res.json({ error: "Username Is Already Used !" });
				}
			}
			...
			bcrypt.hash(password, 12).then((hashedPwd) => {
				const user = new User({
					...
					Fullname: '',
					Address: '',
					Avatar: 'https://i.pinimg.com/736x/a8/57/00/a85700f3c614f6313750b9d8196c08f5.jpg'
				});
		...
		.then((savedUser) => {
			...
			bcrypt.compare(password, savedUser.Password).then((doMatch) => {
				if (doMatch) {
					...
					const token = jwt.sign({ _id: savedUser._id }, "25091997");
					// retrieve the user info details and send it to the front
					const { _id, Name, Email, Followers, Following, Fullname, Address, Avatar, } = savedUser;
					res.json({ token, user: { _id, Name, Email, Followers, Following, Fullname, Address, Avatar, } });
				} else {
					return res.json({
